...
use Absurd\Absurd;
use Absurd\TimeoutError;
use Symfony\Component\Dotenv\Dotenv;
use function Absurd\{awaitEvent, step, sleepFor};
...
$absurd->registerTask('greet-user', handler: function ($params, $ctx) {
    ...
    try {
        echo "[" . date('c') . "] Waiting for event greet:" . $params->userId . "\n";
        $activationPayload = yield awaitEvent('greet:' . $params->userId, ['timeout' => 30]);

        var_dump($activationPayload);
