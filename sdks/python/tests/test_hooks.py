...
class TestBeforeSpawnHook:
    ...
    def test_sync_before_spawn_can_inject_headers(self, conn, queue_name):
        ...
        def before_spawn(
            task_name: str, params: Any, options: SpawnOptions
        ) -> SpawnOptions:
            return {
                **options,
                "headers": {
    ...
    def test_sync_before_spawn_preserves_existing_headers(self, conn, queue_name):
        ...
        assert captured_headers == [
            {"existing": "user-provided", "injected": "by-hook"}
        ]
...
class TestAsyncBeforeSpawnHook:
    ...
    def test_async_before_spawn_can_inject_headers(self, db_dsn, conn, queue_name):
        ...
        async def run():
            client = AsyncAbsurd(
                db_dsn, queue_name=queue, hooks={"before_spawn": before_spawn}
            )
...
class TestWrapTaskExecutionHook:
    ...
    def test_sync_wrap_task_execution(self, conn, queue_name):
        ...
        client = Absurd(
            conn, queue_name=queue, hooks={"wrap_task_execution": wrap_execution}
        )
        client.create_queue()
    ...
    def test_async_wrap_task_execution(self, db_dsn, conn, queue_name):
        ...
        async def run():
            client = AsyncAbsurd(
                db_dsn, queue_name=queue, hooks={"wrap_task_execution": wrap_execution}
            )
        ...
        async def run():
            client = AsyncAbsurd(
                ...
                hooks={
                    ...
                    "wrap_task_execution": wrap_execution,
                },
            )
...
class TestGetCurrentContext:
    ...
    def test_async_concurrent_tasks_have_isolated_contexts(
        self, db_dsn, conn, queue_name
    ):
        ...
        async def run():
            ...
            async def concurrent_test(params, ctx):
                ...
                current = get_current_context()
                captured_task_ids.append(
                    (params["task_num"], current.task_id if current else None)
                )
