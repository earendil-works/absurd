...
const fetchRunsForTask = async (id: string): Promise<TaskSummary[]> => {
	...
	const firstPage = await fetchTasks({ taskId: id, page: 1, perPage: RUNS_PAGE_SIZE })
	...
	for (let currentPage = 2; currentPage <= totalPages; currentPage += 1) {
		const pageResult = await fetchTasks({ taskId: id, page: currentPage, perPage })
			...
			<section class="flex-1 space-y-6 px-6 py-6">
				<Card>
					<CardHeader>
						...
						<CardDescription>
							This task has {runs()?.length ?? 0} recorded run{runs()?.length === 1 ? '' : 's'}.
						</CardDescription>
					</CardHeader>
					<CardContent>
						<Show when={stats().size > 0} fallback={<p class="text-sm text-muted-foreground">No runs have been recorded for this task yet.</p>}>
								...
								{(run) => (
									<Card>
											...
											<CardDescription class="flex flex-wrap gap-2 text-xs">
												...
												<span>Created: {formatRelativeAge(run.createdAt)} ago</span>
												<span>Updated: {formatRelativeAge(run.updatedAt)} ago</span>
											</CardDescription>
										</CardHeader>
										<CardContent class="p-0">
											<TaskDetailView task={run} detail={runDetails()[run.runId]} />
										</CardContent>
									</Card>
								)}
							</For>
