...
import { type TaskDetail, fetchTasks, fetchTask, UnauthorizedError } from '@/lib/api'
...
export default function Tasks(props: TasksProps) {
	...
	const queueOptions = createMemo(() => buildFilterOptions(taskList()?.availableQueues ?? [], 'All queues'))
	const statusOptions = createMemo(() => buildFilterOptions(taskList()?.availableStatuses ?? [], 'All statuses'))
	const taskNameOptions = createMemo(() => buildFilterOptions(taskList()?.availableTaskNames ?? [], 'All task names'))
	const selectedQueueOption = createMemo(() => resolveSelectedOption(queueOptions(), queueFilter()))
	const selectedStatusOption = createMemo(() => resolveSelectedOption(statusOptions(), statusFilter()))
	const selectedTaskNameOption = createMemo(() => resolveSelectedOption(taskNameOptions(), taskNameFilter()))
		...
		<>
			...
			<section class="flex-1 space-y-6 px-6 py-6">
				...
				<Combobox
					...
					onChange={(option) => setQueueFilter(option?.value ? option.value : null)}
				...
				<Select
					...
					onChange={(option) => setStatusFilter(option?.value ? option.value : null)}
				...
				>
					...
					<SelectContent />
							...
							<div class="space-y-1">
								<span class="text-sm font-medium text-foreground">Task name</span>
				<Select
					multiple={false}
					...
					onChange={(option) => setTaskNameFilter(option?.value ? option.value : null)}
					itemComponent={renderSelectOption}
				...
				>
					<SelectTrigger>
						<SelectValue>
							{(state) => {
								const option = state.selectedOption() as FilterOption | undefined
								return option?.label ?? selectedTaskNameOption()?.label ?? 'All task names'
							}}
						</SelectValue>
					</SelectTrigger>
					<SelectContent />
						...
						<div class="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
							<Show when={totalTasks() > 0}>
								<span>
									Showing {pageStart()}–{pageEnd()} of {totalTasks()} task{totalTasks() === 1 ? '' : 's'}
								</span>
							</Show>
											...
											{(task) => (
												<>
													...
													>
														...
														<td class="px-3 py-2">{formatAge(task.createdAt)}</td>
